...
type PeerSidecarConfig struct {
	ChainID compose.ChainID `mapstructure:"chain_id"`
...
type ChainConfig struct {
	ID             compose.ChainID `mapstructure:"id"`
...
func (c *Config) applyEnvOverrides() {
	...
	if chainID > 0 && chainRPC != "" {
		...
		for i := range c.Chains.Chains {
			if c.Chains.Chains[i].ID == compose.ChainID(chainID) {
	...
	for _, suffix := range []string{"A", "B", "C", "D"} {
		...
		if peerAddr != "" {
			...
			c.Peers.Sidecars = append(c.Peers.Sidecars, PeerSidecarConfig{
				ChainID: compose.ChainID(peerChainID),
				Addr:    peerAddr,
			})
		}
	}
...
func (c *Config) GetChainByID(id compose.ChainID) (*ChainConfig, bool) {
	for i := range c.Chains.Chains {
...
func (c *Config) ChainIDs() []compose.ChainID {
	ids := make([]compose.ChainID, len(c.Chains.Chains))
