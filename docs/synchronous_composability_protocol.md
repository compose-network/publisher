...
The communication is assumed to be authenticated and asynchronous. It can be implemented via Protobuf-encoded messages over hardcoded network endpoints provided by an L1 contract.
...
The sequencer only terminates this phase once the simulation returns a successful or failure execution result, with the failure not caused by the Mailbox.
...
> [!NOTE]
> The location of this transaction that populates the Mailbox doesn't need to be strictly top-of-block. For example, it needs to be located after the `Mailbox.clean` transaction. The only requirement is that it happens before the transaction of the cross-chain request.


**SCP Algorithm for the sequencer for a cross-chain transaction with ID xTid**
```
Constants:
...
**SCP Algorithm for the shared publisher for a cross-chain transaction with ID xTid**
```
Constants:
    xTid                    // Cross-chain transaction ID
    timerDuration
