...
func NewMetrics() *Metrics {
	reg := metrics.NewComponentRegistry("publisher", "")
...
// RecordRuntimeMetrics records runtime metrics.
func (m *Metrics) RecordRuntimeMetrics(memAlloc, memSys uint64, goroutines int, gcPause float64) {
	m.RuntimeMemoryAlloc.Set(float64(memAlloc))
	m.RuntimeMemorySys.Set(float64(memSys))
	m.RuntimeGoroutines.Set(float64(goroutines))
	m.RuntimeGCPause.Set(gcPause)
}
