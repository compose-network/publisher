...
- [**Mailbox**](./../../contracts/src/Mailbox.sol): manages message-passing (based on [CIRC](https://espresso.discourse.group/t/circ-coordinated-inter-rollup-communication/43#p-59-mailbox-8)).
- [**PingPong**](./../../contracts/src/PingPong.sol): a contract with two functions:
...
upon receiving a Vote(chainID, vote) message from a sequencer do
    if decision != ⊥:
        return
    ...
    if rollupsThatVoted1 == participatingRollups then do # termination check
        decision ← 1
        send Decided(xTid, 1) to all sequencers
        return
