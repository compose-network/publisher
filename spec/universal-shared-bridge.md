...
Inspired by `OptimismSuperChainERC20`
Should change `OPTIMISM_SUPERCHAIN_ERC20_METADATA_SLOT` AND `Predeploys.SUPERCHAIN_TOKEN_BRIDGE`
...
-   **Token types**
    ...
    -   **CET** (BridgedComposedToken) --- canonical L2 representation
        of L1/L2 asset.
...
function bridgeERC20To(
    ...
    address token,          // native ERC-20 on source L2
    uint256 amount,
    address recipient,      // address on destination L2
    uint256 sessionId
) external;
...
function bridgeCETTo(
    uint256 destinationChainId,
    address bctToken,       // CET on source L2
    uint256 amount,
    address recipient,      // address on destination L2
    uint256 sessionId
) external;
...
function bridgeERC20To(
    ...
    address l1Asset = ERC20Metadata(tokenSrc).l1Source(); // canonical L1 address

    bytes memory payload = abi.encode(sender, receiver, l1Asset, amount);
    ...
    emit TokensSendQueued(chainDest, sender, receiver, l1Asset, amount, sessionId, messageId);
}
```
...
function bridgeCETTo(
    ...
    ICET(bctTokenSrc).crossChainburn(sender, amount);
    emit CETBurned(bctTokenSrc, sender, amount);

    address l1Asset = ICET(bctTokenSrc).l1Source(); // extract canonical L1 address
...
function receiveTokens(
    ...
    (readSender, readReceiver, l1Asset, amount) =
    ...
    address native = nativeTokenOnThisChain(l1Asset);
    if (native != address(0) && IERC20(native).balanceOf(address(this)) >= amount) {
        ...
        token = computeBCTAddress(l1Asset);
