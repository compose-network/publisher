...
To encapsulate all the state transition information about each rollup, the SP will prove a **SuperblockBatch** object, discussed in more detail [below](#superblock).

The validity of rollup blocks is proven building on top of the [Succinct](https://succinctlabs.github.io/op-succinct/introduction.html) framework.
In such framework, once a certain range of blocks is produced, a [STARK](https://eprint.iacr.org/2018/046.pdf) ZK proof is generated, validating the entire range.
...
Block time is set to 12 seconds according to the [SBCP](./superblock_construction_protocol_v2.md) protocol.
...
Note that this defines a batch in terms of time, and not on a number of blocks.
Thus, even if a rollup misses a block, it will still prove its batch for the correct time window.

> [!TIP]
> Why such synchronization is necessary?<br>
...
> [!TIP]
> While this batching methodology may seem odd, it's aligned with the ZK programs standards for proving a longer time-window of changes in a space-optimized manner.
...
2. **Base State Consistency**: ensure that the previous anchored superblock is really the last one submitted to chain.
...
The contract needs to ensure that the base grounds on which the proof was generated is consistent with what was last submitted to chain.
...
To verify the proof, the contract calls the SP1 contract's verification function, providing it:
...
> However, this is useful for the ZK proof to claim, in a space-optimized manner, that there exists a sequence of valid blocks that started from `l2PostRoot` and terminated at `l2PostRoot`.
...
- [`AggregationInputs`](https://github.com/succinctlabs/op-succinct/blob/b6a67787ba53b8f8251ce8b3ffaf0fe2d0d1294f/utils/client/src/types.rs#L8), that contains a list of `BootInfoStruct` objects (committed by the last program)
...
Note that the list of all chain-specific roots should be related to specifically the mailbox root of the last range program, and the ones from previous ranges can be ignored.
...
Soft-confirmations can also be applied to our scenario, though they either don't produce the same confidence level or requires more computational work than in normal settings.
Namely, providing a TEE for an L2 block increases the chance that there will be a valid ZK proof for such a block, but still the superblock can fail due to any other reason (as a missing proof for another block, or invalid Mailbox consistency proof).
...
In this approach, the state and its ZKP is atomically submitted within a single transaction.
- **Pros**:
...
- **Pros**:
  ...
  - Easier to prove correctness of rollback and timeout logic.
- **Cons**:
...
- **Opt-out with isolation**. If a chain fails to prove by the timeout, the set completes without that chain if it didn't interact with any other proved chain.
