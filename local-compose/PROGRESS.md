...
- Python CLI scaffolding is in place and working for non-contract workflows.
  - `DEPLOYMENT_TARGET=calldata ./compose up --fresh --skip-contracts` completes successfully.
- `compose.py status` now summarises service health (including RPC block heights) and no longer crashes. When services are intentionally disabled (e.g., calldata runs), it reads `networks/bootstrap-meta.json` and skips RPC probes with an explicit note.
  - Output now uses PrettyTable (no Rich dependency) with publisher/services/contracts sections matching the requested layout.
  - `compose.py purge --force` removes generated artefacts, leveraging a Docker cleanup fallback for root-owned cache files.
- Full contract deployment (`compose.py up --fresh`) now succeeds consistently when services run. If op-geth returns `transaction indexing is in progress`, we reuse the broadcast artifacts generated by Forge instead of failing the bootstrap. When `DEPLOYMENT_TARGET=calldata` the CLI skips service startup (and therefore helper deployment) unless `COMPOSE_FORCE_SERVICES=1` is present; this mode is for offline/CI artifact generation only and does not yield a working testnet.
- Bootstrap stages are instrumented; as of 2025-09-24 a live `compose.py up --fresh` takes ~215 s end-to-end (op-deployer ≈85 s, helper deployments ≈70 s once Docker layers are cached).
- Repository sync now clones `op-geth` and `optimism` in parallel, and the optimism services share a single multi-target Dockerfile (op-node/op-batcher/op-proposer) so we only compile Go binaries once per build.
...
1. Install dependencies: run `./compose` once; the wrapper will create `.venv` and install Typer/PrettyTable/python-dotenv automatically on first use.
...
3. Typical bootstrap (no L1 writes):
   ```sh
   DEPLOYMENT_TARGET=calldata ./compose up --fresh --skip-contracts
   ./compose status
   ```
4. Full workflow (with helper contracts):
   ```sh
   COMPOSE_FORCE_SERVICES=1 DEPLOYMENT_TARGET=calldata ./compose up --fresh
   ```
