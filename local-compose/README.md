...
3. Confirm the stack is healthy with `./local.sh status`. You should see advancing block numbers for both rollups, a `200` shared publisher health check, and live Blockscout explorers for each chain.
...
- Rebuild and restart specific components with `./local.sh deploy <op-geth|publisher|blockscout|all>`.
...
- `./local.sh deploy blockscout` – restart the explorer + backing services for both rollups.
