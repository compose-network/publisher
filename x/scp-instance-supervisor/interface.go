...
type SCPSupervisor interface {
	// StartInstance attempts to start a new SCP instance (with SCPFactory), returning
	// an error if an instance with the same ID is already active.
...
type SCPFactory func(
	instance compose.Instance, network scp.PublisherNetwork, logger zerolog.Logger,
